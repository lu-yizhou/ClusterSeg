...
class LovaszHinge(torch.nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        def mean(l, ignore_nan=False, empty=0):
            ...
            return acc / n
        if self.per_image:
            loss = mean(self.lovasz_hinge_flat(*self.flatten_binary_scores(log.unsqueeze(0), lab.unsqueeze(0), self.ignore))
                        for log, lab in zip(logits, labels))
        else:
            loss = self.lovasz_hinge_flat(*self.flatten_binary_scores(logits, labels, self.ignore))
        return loss
